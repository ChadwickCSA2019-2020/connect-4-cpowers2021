...
public class MyAgent extends Agent {
  ...
  public void move(){
    ...
    if (!firstMoveMade){
      ...
      if (iCanWin() != -1) {
        ...
        for (int i = 0; i < 4; i++) {
          int checkRight = 3+i;
          int lowestEmptySlotIndexRight = getLowestEmptyIndex(myGame.getColumn(checkRight));
          int checkLeft = 3-i;
          int lowestEmptySlotIndexLeft = getLowestEmptyIndex(myGame.getColumn(checkLeft));
          if ((noGo.get(checkRight) != -1) && (myGame.boardFull() == false) && (lowestEmptySlotIndexRight != -1)) {
            moveOnColumn(checkRight);
            break;
          }
          else if ((noGo.get(checkLeft) != -1) && (myGame.boardFull() == false) && (lowestEmptySlotIndexLeft != -1)) {
            moveOnColumn(checkLeft);
            break;
          }
        }
      }
  ...
  public ArrayList<Integer> weDie() {
    ...
    for (int i = 0; i < 7; i++) {
      ...
      else if (agentMeCopy.theyCanWin() == -1) {
        avoid.add(i);
      }
    }
    return avoid;
